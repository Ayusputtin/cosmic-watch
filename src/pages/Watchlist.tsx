...
export default function Watchlist() {
  ...
  useEffect(() => {
    ...
    return () => {
      clearTimeout(timer);
      if (bgRef.current && (bgRef.current as any).__cleanup) {
        (bgRef.current as any).__cleanup();
      }
    };
  }, [isLoading]);
    ...
    return (
      <div className="cw-page3d bg-background flex items-center justify-center h-screen">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-primary font-display tracking-widest animate-pulse">LOADING SURVEILLANCE DATA...</p>
        </div>
      </div>
    );
  }
        ...
        <main className="relative pt-24 pb-12 px-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-muted-foreground">
                Objects under continuous monitoring • orbital paths • risk drift
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <HologramCard accent="rgba(255,165,55,0.22)">
              ...
              <div className="cw-holoValue">{watchedAsteroids.filter((a) => a.isHazardous).length}</div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredWatched.map((asteroid) => (
                  <AsteroidTile
                    ...
                    isWatched={true}
                    onWatch={toggleWatch}
                    onChat={() => setActiveChat({ id: asteroid.id, name: asteroid.name })}
                  />
                ))}
              </div>
            ) : watchedAsteroids.length === 0 ? (
